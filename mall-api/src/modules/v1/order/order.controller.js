...
const getOrderList = async (req, res, next) => {
  ...
  const {
    start_time, end_time, collect_status, pay_status, order_number
  } = req.query;
  ...
  if (start_time) {
    _sql += ` and a.create_time >= '${start_time}'`;
  }
  if (end_time) {
    _sql += ` and a.create_time <= '${end_time} 23:59:59'`;
  }
  if (collect_status) {
    _sql += ` and a.collect_status = '${collect_status}'`;
  }
  if (pay_status) {
    _sql += ` and a.pay_status = '${pay_status}'`;
  }
  if (order_number) {
    _sql += ` and a.order_number = '${order_number}'`;
  }
  const _countSql = `select count(*) as count from (${_sql}) a`;
  _sql += ` order by a.create_time desc limit ${start}, ${pageSize}`;
  log.info(_sql);
  try {
...
const spitStockSql = async (num, id) => {
  const _update = `update product_specifications set stock= stock - ${num} where id = ${id}`;
  log.info(_update);
...
const insertOrder = async (req, res, next) => {
  const { openid } = req;
  const {
    allPrice, collect_name, address, phone, productList, chooseId, addressId, form_id, account_id, productInfo
  } = req.body;

  // const {specifications_name,number,price} = productList;
  try {
    const orderNumberRows = `${new Date().getTime()}${Math.floor(
      (Math.random() + Math.floor(Math.random() * 9 + 1)) * Math.pow(10, 8 - 1),
    )}`;
    _sql = `insert into account_order(account_id,order_number,should_price,
      collect_name,address,phone,create_time,pay_status)
      values(?,?,?,?,?,?,?,?)`;
    const params = [account_id, orderNumberRows, allPrice, collect_name, address, phone, new Date(), 1];
    ...
    productList.map((item, index) => {
      values.push([orderRows.insertId, item.product_id, item.name, item.number, item.current_price, date]);
    ...
    const addressParam = [collect_name, address, phone, addressId];
...
const getProgramOrderList = async (req, res, next) => {
  ...
  // 获取用户角色
  const userSql = `select is_admin from account where id = ${account_id}`;
  const isAdmin = await pool.query(userSql);
  ...
  if (type == 0) {
    ...
    // 带付款
    _sql += ` and account_id = ${account_id} and pay_status = 1`;
  } else if (type == 2) {
    if (isAdmin === 0) {
      _sql += ` and account_id = ${account_id}`;
    }
    // 管理员能看到所有订单
    // 待发货
    _sql += ' and pay_status = 2 and collect_status = 1';
  } else if (type == 3) {
    _sql += ` and account_id = ${account_id}  and pay_status = 2 and collect_status = 2`;
  }
  const _countSql = `select count(*) as count from (${_sql}) a`;
  _sql += ` order by create_time desc limit ${start}, ${pageSize}`;
  log.info(_sql);
  try {
    const rows = await pool.query(_sql);
    const counts = await pool.query(_countSql);
...
const deliverGoods = async (req, res, next) => {
  ...
  try {
    let sql = `update account_order set
      ...
      ship_time=?
      where order_number = ?`;
    const params = [logistics_name, logistics_number, new Date(), order_number];
    sql = mysql.format(sql, params);
    log.info(sql);
    await pool.query(sql);
...
const getAllOrderInfo = async (req, res, next) => {
  try {
    ...
    const pendingPayment = rows[0].count;
    // 待发货
    if (isAdmin === 0) {
